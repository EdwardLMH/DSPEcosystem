...
public final class TealiumClient {
    ...
    private func flushBatch() {
        ...
        session.dataTask(with: req) { [weak self] _, _, err in
            if err != nil {
                // Re-queue on failure (best-effort, capped to 200)
                self?.queue.async {
                    self?.batch.insert(contentsOf: events, at: 0)
                    if (self?.batch.count ?? 0) > 200 {
                        self?.batch = Array((self?.batch.suffix(200))!)
                    }
                }
            }
        }.resume()
