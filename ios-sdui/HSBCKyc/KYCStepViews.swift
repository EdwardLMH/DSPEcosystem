...
struct KYCIdentifierStep: View {
    // BFF step-004 expects: q_hkid_number (required) + q_hkid_expiry (required)
    @Environment(AppStore.self) private var store
    @State private var hkidNumber = ""
    @State private var hkidExpiry = ""
    @State private var numberErr: String?
    @FocusState private var focused: Field?
    enum Field { case number, expiry }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            FieldGroup(title: "HKID Number", required: true,
                ...
                TextField("A123456(7)", text: $hkidNumber)
                    .hiveInput(focused: focused == .number, error: numberErr != nil)
                    .focused($focused, equals: .number)
                    .autocapitalization(.allCharacters)
                    .disableAutocorrection(true)
                    .onChange(of: hkidNumber) { _, v in
                        numberErr = nil
                        // Send immediately on every change so Continue works
                        store.dispatch(.setAnswer(questionId: "q_hkid_number", value: AnyCodable(v)))
                    }
            }

            FieldGroup(title: "HKID Expiry Date",
                       help: "Enter N/A if your HKID has no expiry date") {
                TextField("DD/MM/YYYY or N/A", text: $hkidExpiry)
                    .hiveInput(focused: focused == .expiry)
                    .focused($focused, equals: .expiry)
                    .disableAutocorrection(true)
        ...
        .onAppear {
            // Pre-fill expiry with N/A so Continue doesn't block if user skips
            if hkidExpiry.isEmpty {
                hkidExpiry = "N/A"
                store.dispatch(.setAnswer(questionId: "q_hkid_expiry", value: AnyCodable("N/A")))
            }
        }
    }
}
...
struct KYCAddressStep: View {
    ...
    private func sendAddress() {
        // BFF step-007 expects q_addr_line1 and q_addr_line2
        let line1 = [flat, floor, block].filter { !$0.isEmpty }.joined(separator: " ")
        let line2 = [building, street].filter { !$0.isEmpty }.joined(separator: ", ")
        store.dispatch(.setAnswer(questionId: "q_addr_line1", value: AnyCodable(line1.isEmpty ? street : line1)))
        store.dispatch(.setAnswer(questionId: "q_addr_line2", value: AnyCodable(line2)))
    }
}
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showFrontPicker) {
            ImageCaptureSheet { img in
                frontImage = img
                store.dispatch(.setAnswer(questionId: "q_hkid_front",
    ...
    private var instructionsView: some View {
        VStack(spacing: Hive.Spacing.s4) {
            HiveInfoBanner(
                ...
                bodyText: "We need to verify you are a real person matching your ID. This takes ~30 seconds."
            )
            ForEach([
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            HiveInfoBanner(
                ...
                bodyText: "We use Open Banking (FCA/HKMA regulated) to verify your identity. Your bank credentials are never shared with HSBC."
            )
...
// MARK: - Split step views for new SDUI router


struct KYCNameDobStep: View {
    @Environment(AppStore.self) private var store
    @State private var firstName = ""
    @State private var lastName  = ""
    @State private var dob = Date()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            FieldGroup(title: "First Name / Given Name", required: true, help: "As on your official ID") {
                TextField("e.g. Tai Man", text: $firstName)
                    .hiveInput(focused: focus == .first)
                    .focused($focus, equals: .first)
                    .textContentType(.givenName).autocapitalization(.words)
                    .onChange(of: firstName) { _, v in store.dispatch(.setAnswer(questionId: "q_first_name", value: AnyCodable(v))) }
            }
            FieldGroup(title: "Last Name / Surname", required: true) {
                TextField("e.g. CHAN", text: $lastName)
                    .hiveInput(focused: focus == .last)
                    .focused($focus, equals: .last)
                    .textContentType(.familyName).autocapitalization(.allCharacters)
                    .onChange(of: lastName) { _, v in store.dispatch(.setAnswer(questionId: "q_last_name", value: AnyCodable(v))) }
            }
            FieldGroup(title: "Date of Birth", required: true, help: "Must be 18 or older") {
                DatePicker("", selection: $dob, in: ...maxDob, displayedComponents: .date)
                    .datePickerStyle(.compact).labelsHidden()
                    .frame(maxWidth:.infinity, alignment:.leading)
                    .padding(.horizontal, Hive.Spacing.s4)
                    .frame(height: Hive.Component.Input.height)
                    .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
                    .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
                    .onChange(of: dob) { _, d in
                        let f = DateFormatter(); f.dateFormat = "yyyy-MM-dd"
                        store.dispatch(.setAnswer(questionId: "q_date_of_birth", value: AnyCodable(f.string(from: d))))
                    }
            }
        }
    }
...
struct KYCNationalityStep: View {
    @Environment(AppStore.self) private var store
    @State private var nationality = ""
    ...
    var body: some View {
        FieldGroup(title: "Nationality", required: true) {
            Button(action: { showSheet = true }) {
                HStack {
                    Group {
                        if nationality.isEmpty {
                            Text("Select nationality").foregroundColor(Hive.Color.n400)
                        } else {
                            let c = KYC_COUNTRIES.first { $0.iso2 == nationality }
                            Text("\(c?.flag ?? "") \(c?.name ?? nationality)").foregroundColor(Hive.Color.n800)
                        }
                    }
                    Spacer()
                    Image(systemName: "chevron.down").font(.system(size:12)).foregroundColor(Hive.Color.n400)
                }
                .padding(.horizontal, Hive.Spacing.s4)
                .frame(height: Hive.Component.Input.height)
                .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
        ...
        .sheet(isPresented: $showSheet) {
            CountryPickerSheet(selected: $nationality) { iso2 in
                nationality = iso2
                store.dispatch(.setAnswer(questionId: "q_nationality", value: AnyCodable(iso2)))
                showSheet = false
            }
...
struct KYCAddressDistrictStep: View {
    @Environment(AppStore.self) private var store
    @State private var district = ""
    @State private var region   = ""
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            FieldGroup(title: "District", required: true) {
                Picker("District", selection: $district) {
                    Text("Select district").tag("")
                    ForEach(HK_DISTRICTS, id: \.0) { d in Text(d.1).tag(d.0) }
                }
                .pickerStyle(.menu).frame(maxWidth:.infinity,alignment:.leading)
                .padding(.horizontal, Hive.Spacing.s4).frame(height: Hive.Component.Input.height)
                .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
                .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
                .onChange(of: district) { _, v in store.dispatch(.setAnswer(questionId: "q_addr_district", value: AnyCodable(v))) }
            }
            FieldGroup(title: "Region", required: true) {
                ...
                .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
                .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
                .onChange(of: region) { _, v in store.dispatch(.setAnswer(questionId: "q_addr_region", value: AnyCodable(v))) }
            }
        }
    }
}

struct KYCEmploymentStep: View {
    @Environment(AppStore.self) private var store
    ...
    var body: some View {
        FieldGroup(title: "Employment Status", required: true) {
            Picker("Status", selection: $status) {
                Text("Select employment status").tag("")
            ...
            .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
            .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
            .onChange(of: status) { _, v in store.dispatch(.setAnswer(questionId: "q_employment_status", value: AnyCodable(v))) }
        }
    }
}
...
struct KYCSourceOfFundsStep: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            FieldGroup(title: "Primary Source of Funds", required: true) {
                Picker("Source", selection: $fundsSource) {
                    Text("Select source of funds").tag("")
                    ForEach(FUNDS_SOURCES, id: \.0) { o in Text(o.1).tag(o.0) }
                }
                .pickerStyle(.menu).frame(maxWidth:.infinity,alignment:.leading)
                .padding(.horizontal, Hive.Spacing.s4).frame(height: Hive.Component.Input.height)
                .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
                .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
                .onChange(of: fundsSource) { _, v in store.dispatch(.setAnswer(questionId: "q_source_of_funds", value: AnyCodable(v))) }
            }
            FieldGroup(title: "Purpose of Account", required: true) {
                Picker("Purpose", selection: $purpose) {
                    Text("Select account purpose").tag("")
                    ForEach(ACCOUNT_PURPOSES, id: \.0) { o in Text(o.1).tag(o.0) }
                }
                .pickerStyle(.menu).frame(maxWidth:.infinity,alignment:.leading)
                .padding(.horizontal, Hive.Spacing.s4).frame(height: Hive.Component.Input.height)
                .background(Hive.Color.brandWhite).cornerRadius(Hive.Radius.base)
                .overlay(RoundedRectangle(cornerRadius: Hive.Radius.base).stroke(Hive.Color.n300, lineWidth:1))
                .onChange(of: purpose) { _, v in store.dispatch(.setAnswer(questionId: "q_account_purpose", value: AnyCodable(v))) }
            }
        }
    }
}

// Annual income as its own step (mobile splits employment into two steps)
struct KYCAnnualIncomeStep: View {
    @Environment(AppStore.self) private var store
    @State private var income = ""
    private let options = [
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
            HiveFieldLabel(text: "Annual Income (HKD)", required: true)
            Picker("Income", selection: $income) {
                Text("Select income range").tag("")
                ForEach(options, id: \.0) { o in Text(o.1).tag(o.0) }
            }
            .pickerStyle(.menu)
            .frame(maxWidth: .infinity, alignment: .leading)
...
// Standalone Date of Birth step (when BFF splits DOB onto its own mobile step)
struct KYCDateOfBirthStep: View {
    @Environment(AppStore.self) private var store
    @State private var dob = Date()
    private var maxDob: Date {
        Calendar.current.date(byAdding: .year, value: -18, to: Date()) ?? Date()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
            HiveFieldLabel(text: "Date of Birth", required: true)
            DatePicker("", selection: $dob, in: ...maxDob, displayedComponents: .date)
                .datePickerStyle(.wheel)
                .labelsHidden()
                .frame(maxWidth: .infinity)
                .padding(.vertical, Hive.Spacing.s2)
                .onChange(of: dob) { _, d in
                    let f = DateFormatter()
                    f.dateFormat = "yyyy-MM-dd"
                    store.dispatch(.setAnswer(questionId: "q_date_of_birth",
                                              value: AnyCodable(f.string(from: d))))
                }
            HiveHelpText(message: "You must be at least 18 years old to apply")
        }
        .onAppear {
            // Pre-fill with a default valid date so Continue works immediately
            let f = DateFormatter()
            f.dateFormat = "yyyy-MM-dd"
            store.dispatch(.setAnswer(questionId: "q_date_of_birth",
                                      value: AnyCodable(f.string(from: maxDob))))
        }
    }
}

// Account purpose step (step-012: q_account_purpose alone)
struct KYCAccountPurposeStep: View {
    @Environment(AppStore.self) private var store
    @State private var purpose = ""

    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
            HiveFieldLabel(text: "Purpose of Account", required: true)
            HiveHelpText(message: "How do you primarily plan to use this account?")
            Picker("Purpose", selection: $purpose) {
                Text("Select account purpose").tag("")
                ForEach(ACCOUNT_PURPOSES, id: \.0) { o in Text(o.1).tag(o.0) }
            }
...
// FATCA declaration step (step-016: decl_fatca alone — final step)
struct KYCFatcaStep: View {
    @Environment(AppStore.self) private var store
    @State private var fatcaAgreed = false
    @State private var finalAgreed = false
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s5) {
            ...
            checkRow(
                id: "decl_fatca",
                text: "I confirm I am NOT a US person for FATCA purposes (no US citizenship, Green Card, or US tax residency). If you are a US person, please contact us before proceeding.",
                checked: $fatcaAgreed
            )

            // Final truthfulness declaration
            checkRow(
                id: "decl_truthful",
                text: "By submitting this application, I confirm all information provided is true and complete. I consent to HSBC processing my personal data per PDPO Cap. 486 and HSBC's Privacy Notice.",
                checked: $finalAgreed
            )

            Text("Your declarations are legally binding under Hong Kong law.")
                .font(Hive.Typography.caption)
                .foregroundColor(Hive.Color.n400)
                .padding(Hive.Spacing.s4)
                .frame(maxWidth: .infinity, alignment: .leading)
    ...
    @ViewBuilder
    private func checkRow(id: String, text: String, checked: Binding<Bool>) -> some View {
        Button(action: {
            checked.wrappedValue.toggle()
            store.dispatch(.setAnswer(questionId: id, value: AnyCodable(checked.wrappedValue)))
        }) {
            HStack(alignment: .top, spacing: Hive.Spacing.s3) {
                ZStack {
                    RoundedRectangle(cornerRadius: Hive.Radius.sm)
                        .stroke(checked.wrappedValue ? Hive.Color.success : Hive.Color.n300, lineWidth: 2)
                        .frame(width: 22, height: 22)
                ...
                .animation(.easeInOut(duration: 0.15), value: checked.wrappedValue)
            ...
            .padding(Hive.Spacing.s4)
            .background(checked.wrappedValue ? Hive.Color.successLight : Hive.Color.n50)
            .cornerRadius(Hive.Radius.md)
            .overlay(RoundedRectangle(cornerRadius: Hive.Radius.md)
                .stroke(checked.wrappedValue ? Hive.Color.success : Hive.Color.n200, lineWidth: 1))
        }
        .buttonStyle(.plain)
        .animation(.easeInOut(duration: 0.15), value: checked.wrappedValue)
    }
...
// MARK: - Nationality-aware ID steps

// HK — HKID number + expiry
struct KYCHKIDStep: View {
    @Environment(AppStore.self) private var store
    @State private var hkidNumber = ""
    @State private var hkidExpiry = "N/A"
    @FocusState private var focused: Field?
    enum Field { case number, expiry }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
                HiveFieldLabel(text: "HKID Number", required: true)
                HiveHelpText(message: "Include check digit in brackets — e.g. A123456(7)")
                TextField("A123456(7)", text: $hkidNumber)
                    .hiveInput(focused: focused == .number)
                    .focused($focused, equals: .number)
                    .textCase(.uppercase)
                    .autocapitalization(.allCharacters)
                    .disableAutocorrection(true)
            ...
            VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
                HiveFieldLabel(text: "HKID Expiry Date")
                HiveHelpText(message: "Enter N/A for permanent residents with no expiry")
                TextField("DD/MM/YYYY or N/A", text: $hkidExpiry)
                    .hiveInput(focused: focused == .expiry)
                    .focused($focused, equals: .expiry)
                    .disableAutocorrection(true)
        ...
        .onAppear {
            // Pre-seed expiry so Continue works without touching this field
            store.dispatch(.setAnswer(questionId: "q_hkid_expiry", value: AnyCodable("N/A")))
        }
    }
}

// Mainland China — 18-digit resident ID
struct KYCMainlandIDStep: View {
    @Environment(AppStore.self) private var store
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
            HiveFieldLabel(text: "Mainland China Resident ID (居民身份証)", required: true)
            HiveHelpText(message: "18-digit ID number on your 居民身份证")
            TextField("110101199001011234", text: $idNumber)
                .hiveInput(focused: focused)
                .focused($focused)
            ...
            if !idNumber.isEmpty && idNumber.count != 18 {
                HiveErrorText(message: "Mainland ID must be 18 digits")
            }
        }
    }
}

// Passport — number + expiry (for SG, GB, US, and all other nationalities)
struct KYCPassportStep: View {
    @Environment(AppStore.self) private var store
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Hive.Spacing.s4) {
            VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
                HiveFieldLabel(text: "Passport Number", required: true)
                TextField("e.g. X12345678", text: $passportNumber)
                    .hiveInput(focused: focused == .number)
                    .focused($focused, equals: .number)
                    .autocapitalization(.allCharacters)
                    .disableAutocorrection(true)
                    .onChange(of: passportNumber) { _, v in
                        store.dispatch(.setAnswer(questionId: "q_passport_number", value: AnyCodable(v)))
                    }
            }
            VStack(alignment: .leading, spacing: Hive.Spacing.s1) {
                HiveFieldLabel(text: "Passport Expiry Date", required: true)
                TextField("DD/MM/YYYY", text: $passportExpiry)
                    .hiveInput(focused: focused == .expiry)
                    .focused($focused, equals: .expiry)
                    .disableAutocorrection(true)
                    .onChange(of: passportExpiry) { _, v in
