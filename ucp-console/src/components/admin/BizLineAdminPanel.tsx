...
import type { BizLine } from '../../types/ucp';
...
export function BizLineAdminPanel() {
  ...
  function save() {
    ...
    const biz: BizLine = { bizLineId: form.bizLineId!, displayName: form.displayName!, description: form.description ?? '', active: !!form.active };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--surface-bg)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '20px 24px', background: 'var(--surface-panel)', borderBottom: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>Business Line Administration</h1>
          <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '3px 0 0' }}>Manage business lines used to scope UCP content and entitlements</p>
        </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowForm(false)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: 440, boxShadow: 'var(--shadow-xl)' }} onClick={e => e.stopPropagation()}>
            ...
            {[
              { field: 'bizLineId',    label: 'Biz Line ID', disabled: !!editing },
              { field: 'displayName',  label: 'Display Name', disabled: false },
              { field: 'description',  label: 'Description', disabled: false },
            ].map(({ field, label, disabled }) => (
              <div key={field} style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>{label}</label>
                <input
                  value={(form as Record<string, string>)[field] ?? ''}
                  onChange={e => setForm(f => ({ ...f, [field]: e.target.value }))}
                  disabled={disabled}
                  style={{ ...inp, background: disabled ? '#F9FAFB' : '#fff', color: disabled ? '#9CA3AF' : '#111' }}
                />
              </div>
            ))}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={save} disabled={!form.bizLineId || !form.displayName}
                style={{ padding: '9px 20px', background: form.bizLineId && form.displayName ? 'var(--hsbc-red)' : '#F3F4F6', color: form.bizLineId && form.displayName ? '#fff' : '#9CA3AF', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: form.bizLineId && form.displayName ? 'pointer' : 'not-allowed' }}>
                Save
              </button>
            </div>
          </div>
