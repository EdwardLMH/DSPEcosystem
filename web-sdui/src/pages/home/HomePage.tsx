...
function useHomeSDUI() {
  const { bffHeaders } = useSDUIContext();
  const [loadState, setLoadState] = useState<LoadState>('loading');
  const [slices, setSlices] = useState<HomeSlice[]>([]);
...
function SDUISliceView({ slice }: { slice: HomeSlice }) {
  const p = slice.props ?? {};
  switch (slice.type) {
    ...
    <div style={{ background: bg, padding: '10px 14px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <span
            ...
            onClick={() => tealium.track({
              ...
              event_action: 'notification_tapped', screen_name: 'home_hub_hk', journey_name: 'home_hub',
            })}
          >🔔</span>
          <span
            style={{ fontSize: 20, cursor: 'pointer', color: '#fff' }}
            onClick={() => tealium.track({
              ...
              event_action: 'headset_tapped', screen_name: 'home_hub_hk', journey_name: 'home_hub',
            })}
          >🎧</span>
        </div>
      </div>
      ...
      <div
        ...
        onClick={() => tealium.track({
          ...
          event_action: 'search_tapped', screen_name: 'home_hub_hk', journey_name: 'home_hub',
        })}
      >
