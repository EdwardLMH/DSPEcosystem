...
export class HomePageView extends ViewPU {
    constructor(parent, params, __localStorage, elmtId = -1, paramsLambda = undefined, extraInfo) {
        super(parent, __localStorage, elmtId, extraInfo);
        if (typeof paramsLambda === "function") {
            this.paramsGenerator_ = paramsLambda;
    ...
    private __sdui: ObservedPropertyObjectPU<HomeSlice[]>; // BFF / static slices when loaded
    get sdui() {
        return this.__sdui.get();
    }
    set sdui(newValue: HomeSlice[]) {
    ...
    private async mergeSelfPick(slices: HomeSlice[]): Promise<void> {
        for (let i = 0; i < slices.length; i++) {
            ...
            if (slice.type === 'SELF_PICK_ENTRY_POINTS' || slice.type === 'COMBO_QUICK_ACCESS') {
                ...
                const merged = await this.staticDist.resolvedSelfPickItems('home-hub-hk', remoteRow1, this.selfPickForceUpdate);
